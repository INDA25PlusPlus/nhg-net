...
impl event::EventHandler<ggez::GameError> for MainState {
    ...
    for row in 0..8 {
      for col in 0..8 {
        ...
        let square = graphics::Mesh::new_rectangle(
          ...
          graphics::Rect { x: 0.0, y: 0.0, w: 75.0, h: 75.0 }, 
          if (row + col) % 2 == 0 { graphics::Color::WHITE } else { graphics::Color::BLACK }
        )?;

        canvas.draw(&square, Vec2::new(x, y));

        let pos = Position{row: 7 as i8 - row, col};
        match self.board.get(pos) {
          Some(piece) => {
            let draw_color = match piece.color {
              hermanha_chess::Color::White => {
                "w"
              }
              hermanha_chess::Color::Black => {
                "b"
              }
            };
            ...
            match piece.piece_type {
              hermanha_chess::PieceType::Pawn => {
                ...
                let draw_piece = graphics::Text::new(
                  TextFragment::new("p")
                    .scale(PxScale::from(57.0))
                    .color(if draw_color == "w" { graphics::Color::WHITE } else { graphics::Color::BLACK })
                );
  ...
  fn mouse_button_down_event(
      ...
      let clicked_pos = Position { row: 7 as i8 - row as i8, col: col as i8 };

      println!("selected ? : {} {}", self.selected_piece.row, self.selected_piece.col);
      println!("pos: {} {}", clicked_pos.row, clicked_pos.col);

      match self.board.get(self.selected_piece) {
        Some(found_piece) => {
          println!("if");
          self.board.move_piece(self.selected_piece, clicked_pos, Some(PieceType::Queen));
        }
        None => ()
      }
